...
const Contact = () => {
  const mapLink = "https://maps.google.com/?q=Dream+India+School+Tiruvuru";
  ...
  return (
    <section id="contact" className="py-20 md:py-28 px-6 md:px-12 bg-white relative overflow-hidden border-t border-slate-100">
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 relative z-10">
        
        {/* Left Column: Contact Details */}
        <div className="flex flex-col justify-center text-left">
          <span className="text-blue-600 font-bold tracking-wider uppercase text-xs mb-3 block font-outfit">Get In Touch</span>
          <h2 className="text-3xl md:text-4xl font-outfit font-extrabold text-[#0f172a] mb-4 leading-tight">
          ...
          <div className="space-y-6">
            {contactData.map((item, i) => (
              ...
              >
                ...
                <div>
                  <h4 className="font-outfit font-extrabold text-[#0f172a] text-sm md:text-base mb-0.5">
                    {item.title}
                  </h4>
                  {item.href ? (
                    <a href={item.href} className="text-slate-500 text-xs md:text-sm font-semibold hover:text-blue-600 transition-colors">
                      {item.desc}
                    </a>
                  ) : (
        ...
        <motion.div 
          ...
          className="w-full h-full min-h-[350px] lg:min-h-[450px] aspect-[4/3] rounded-2xl border border-slate-200 overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 relative bg-slate-50 flex"
        >
          <iframe
